...
def _render_meta_cell(cell: Any, *, plant: Optional[Dict[str, Any]], target_date: Optional[date]) -> str:
    text = str(cell or "")
    ...
    *,
    ...
    if output_style == "gsnp_multiline":
        ...
        for row in meta_rows:
            writer.writerow([_render_meta_cell(cell, plant=plant, target_date=target_date) for cell in list(row)])
        for row in rows:
