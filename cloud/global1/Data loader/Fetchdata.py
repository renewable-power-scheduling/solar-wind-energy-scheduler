...
def _metered_name_matches_run_date(name: str, run_date: str) -> bool:
    dt = datetime.strptime(run_date, "%Y-%m-%d")
    tokens = {
        run_date,
        dt.strftime("%Y%m%d"),
...
def _sync_metered(client: RemoteClient, cfg: dict, run_date: str, dirs: dict[str, Path]) -> None:
    ...
    try:
        ...
        fallback_used = True
        stale_fallback = not _metered_name_matches_run_date(fallback_name, run_date)
        if stale_fallback:
            finished_at = datetime.now(IST).isoformat()
            try:
                tmp_file.unlink(missing_ok=True)
            except Exception:
                logger.debug("Could not remove stale metered tmp file: %s", _rel(tmp_file), exc_info=True)
            logger.warning(
                ...
                manifest.setdefault("raw_inputs", {}).setdefault("metered", []).append(
                    {
                        ...
                        "requested_local_path": _rel(dirs["metered"] / filename),
                        "local_path": None,
                        "download_started_at_ist": started_at,
                        "download_finished_at_ist": finished_at,
