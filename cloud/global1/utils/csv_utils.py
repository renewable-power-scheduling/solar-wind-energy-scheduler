...
def _get_site_forecast_column_normalized() -> str | None:
    """
    Reads config/sites/<site>.json using SITE_ID and returns normalized
    enercast.forecast_column when configured.
    """
...
def load_enercast_forecast_csv(path: Path):
    ...
    if header_idx is None:
        # Backward compatibility for old files where header starts after metadata lines
    ...
    # Two-row header support (Availability/Forecast row)
    ...
    if forecast_idx is None:
        # Common Enercast single-header form:
        #   ..., <SITE_FORECAST_COL>, Availability...
        # In that case use column immediately before availability.
    ...
    for line in lines[data_start:]:
        ...
        rows.append({"block": block, "forecast_mw": forecast_mw})
