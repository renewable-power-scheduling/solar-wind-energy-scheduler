...
# Custom Fetch Standard

## Purpose

This script is a command-line utility for fetching date-specific data for a given site and storing it in a custom input directory. It acts as a wrapper around the core `[[fetch_worker.py]]` script, configuring it to run for a specific use case. This "standard" version defaults to the `SIRMOUR` site.

## File Location

```
d:\Vedanjay Power\Codes Deployed on cloud\Test Environment Code\Github code\solar-wind-energy-scheduler-test-env\cloud	ools\custom_runner\custom_fetch_standard.py
```

## Execution Flow

1.  **Parse Arguments**: The script parses the `--date` and `--site` command-line arguments. The site defaults to `SIRMOUR` if not provided or if the `SITE_ID` environment variable is not set.
...
| `--site` | No       | The site ID. Defaults to `SIRMOUR`.          |
