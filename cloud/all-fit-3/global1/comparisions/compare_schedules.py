...
- Optionally overlay actual metered generation.
- Output:
  1) Comparison CSV
  2) Plotly HTML graph
...
2) Hierarchy mode:
   ...
   Expects:
     ...
     <root>/<plant>/<date>/metered_data/ (optional)
   Writes:
...
DEFAULT_METERED_PATH = ""
DEFAULT_OUT_DIR = ""
DEFAULT_DATE_LABEL = "2026-03-13"
DEFAULT_ROOT = os.path.dirname(os.path.abspath(__file__))
DEFAULT_PLANT = "KOTHAGUDEM"  # e.g. GSNP, CME, or leave empty for no plant-specific config
...
def resolve_default_path(path_value: str) -> str:
    ...
    if normalized.lower().startswith(prefix):
        base_dir = os.path.dirname(DEFAULT_ROOT)  # global1/
...
def _block_from_time_text(raw: str) -> Optional[int]:
    if raw is None:
        return None
    text = str(raw).strip()
    if not text:
    ...
    hh = int(m.group(1))
    mm = int(m.group(2))
    if not (0 <= hh <= 23 and mm in (0, 15, 30, 45)):
        return None
    return (hh * 60 + mm) // 15 + 1


def parse_metered_series_map(text: str, site_cfg: Optional[Dict] = None) -> Dict[int, float]:
    lines = [l for l in text.splitlines() if l.strip()]
    if not lines:
        return {}
    ...
    normalized = [to_header_key(h) for h in headers]
    mode_cfg = ((site_cfg or {}).get("comparison", {}) or {}).get("metered", {}) or {}
    ...
    def find_config_col(key: str) -> int:
        configured = mode_cfg.get(key)
        if not configured:
            return -1
        if isinstance(configured, str):
            configured = [configured]
        targets = {to_header_key(v) for v in configured}
        for idx, h in enumerate(normalized):
            if h in targets:
                return idx
        return -1

    block_idx = find_config_col("block_column")
    if block_idx == -1:
        block_idx = find_col(["block", "blockno", "blk"])

    time_idx = find_config_col("time_column")
    if time_idx == -1:
        time_idx = find_col(["timestamp", "time"])

    value_idx = find_config_col("value_column")
    if value_idx == -1:
        value_idx = find_col(
            [
                "activepoweravgmfmoutkw",
                "activepower",
                "actualmetered",
                "actual",
                "metered",
    ...
    for i, cols in enumerate(rows):
        ...
        if block_idx != -1 and block_idx < len(cols):
            blk = parse_block_number(cols[block_idx])
        if blk is None and time_idx != -1 and time_idx < len(cols):
            blk = _block_from_time_text(cols[time_idx])
        if blk is None:
            blk = i + 1
        if not (1 <= blk <= TOTAL_BLOCKS):
            continue
...
def write_comparison_csv(
    ...
    metered_map: Optional[Dict[int, float]] = None,
) -> None:
    metered_map = metered_map or {}
    headers = [
        "Block",
        "Time",
        "System Schedule (MW)",
        ...
        "Abs Diff (MW)",
        "Diff % of System",
        "System vs Metered (MW)",
        "Vedanjay vs Metered (MW)",
    ]
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for block in range(1, TOTAL_BLOCKS + 1):
            ...
            metered_val = metered_map.get(block)
            ...
            sys_metered_diff = (sys_val - metered_val) if (sys_val is not None and metered_val is not None) else None
            vj_metered_diff = (vj_val - metered_val) if (vj_val is not None and metered_val is not None) else None
            writer.writerow(
                [
                    block,
                    build_time(block),
                    ...
                    f"{metered_val:.3f}" if metered_val is not None else "",
                    f"{diff:.3f}" if diff is not None else "",
                    f"{abs_diff:.3f}" if abs_diff is not None else "",
                    f"{pct:.2f}" if pct is not None else "",
                    f"{sys_metered_diff:.3f}" if sys_metered_diff is not None else "",
                    f"{vj_metered_diff:.3f}" if vj_metered_diff is not None else "",
                ]
            )
...
def write_plot_html(
    ...
    y_metered = [None if metered_map is None else metered_map.get(b) for b in x]
...
def resolve_hierarchy_paths(root: str, plant: str, date_s: str) -> Tuple[str, str, str, str]:
    base = os.path.join(root, plant.upper(), date_s)
    system_dir = os.path.join(base, "System")
    vedanjay_dir = os.path.join(base, "vedanjay")
    metered_dir = os.path.join(base, "metered_data")
    out_dir = os.path.join(base, "compared")
    return system_dir, vedanjay_dir, metered_dir, out_dir
...
def main() -> None:
    ...
    parser.add_argument("--metered", default=None, help="Path to metered file or directory")
    ...
    metered_path = args.metered or resolve_default_path(DEFAULT_METERED_PATH)
    ...
    if not system_path and not vedanjay_path and not out_dir and plant_name and date_label:
        h_system, h_vedanjay, h_metered, h_out = resolve_hierarchy_paths(root_path, plant_name, date_label)
        system_path = h_system
        vedanjay_path = h_vedanjay
        metered_path = h_metered
        out_dir = h_out

    sys_file = pick_latest_file(system_path)
    vj_file = pick_latest_file(vedanjay_path)
    metered_file = None
    if metered_path and os.path.exists(metered_path):
        try:
            metered_file = pick_latest_file(metered_path)
    ...
    metered_text = read_text_or_excel(metered_file) if metered_file else ""
    ...
    metered_map = parse_metered_series_map(metered_text, site_cfg=site_cfg) if metered_text else {}
    ...
    write_comparison_csv(csv_out, system_map, vedanjay_map, metered_map)
    ...
    print("Metered file:", metered_file if metered_file else "Not found")
    print("CSV:", csv_out)
    print("HTML:", html_out)
