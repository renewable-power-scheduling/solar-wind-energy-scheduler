...
# Shared session sites are configurable per deployment bundle.
...
def _load_fetchdata_module():
    ...
    script_path = Path(__file__).resolve().parents[1] / "fetcher_core" / "fetch_worker.py"
    spec = importlib.util.spec_from_file_location("global1_fetchdata_shared", script_path)
...
def _run_fetch_once(site_name: str) -> subprocess.CompletedProcess:
    script = Path(__file__).resolve().parents[1] / "fetcher_core" / "fetch_worker.py"
...
def _arrival_timestamp_ist_from_filename(name: str) -> str | None:
    base = Path(name).name
    m = re.search(
        r"(?P<date>\d{4}-\d{2}-\d{2})-(?P<hh>\d{2})-(?P<mm>\d{2})(?:\+|\-)\d{4}(?=\.csv$)",
        base,
        re.IGNORECASE,
    )
    if not m:
    ...
    if not (0 <= hh <= 23 and 0 <= mm <= 59):
        return None
    return f"{dt_date} {hh:02d}:{mm:02d}:00"
    ...
    try:
        ...
        for site in sites:
            ...
            try:
                ...
                logger.exception("Fetch failed for site=%s", site)
                ok = False
                any_failed = True
                stderr_tail = str(exc)
