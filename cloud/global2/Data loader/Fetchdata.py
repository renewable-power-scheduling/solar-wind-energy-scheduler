...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--site", default=os.getenv("SITE_ID", "GSNP"), help="Site ID, e.g. SIRMOUR or GSNP")
...
def _parse_forecast_meta(
    ...
    *,
    ...
    if kind_override:
        if len(groups) >= 4:
            kind_raw = str(groups[0]).strip().lower()
            if kind_raw in {"intraday", "dayahead"}:
                kind_override = None
        if kind_override:
            if len(groups) < 3:
                return None
            date_s, hh, mm = groups[0], groups[1], groups[2]
            if date_s != run_date:
                return None
            ts = datetime.strptime(f"{date_s} {hh}:{mm}", "%Y-%m-%d %H:%M")
            return {"kind": kind_override, "sort": ts, "name": name}
...
def _sync_forecasts(client: RemoteClient, cfg: dict, run_date: str, dirs: dict[str, Path]) -> None:
    ...
    intraday_dir = paths_cfg.get("remote_forecasts")
    intraday_rx = pattern_cfg.get("forecast_regex")
    if intraday_dir and intraday_rx:
        sources.append(
            {
                "remote_dir": intraday_dir,
                "regex": re.compile(intraday_rx, re.IGNORECASE),
                "kind_override": None,
            }
        )

    dayahead_dir = paths_cfg.get("remote_forecasts_dayahead")
    dayahead_rx = pattern_cfg.get("dayahead_regex") or intraday_rx
    if dayahead_dir and dayahead_rx:
        sources.append(
            {
                "remote_dir": dayahead_dir,
                "regex": re.compile(dayahead_rx, re.IGNORECASE),
                "kind_override": "dayahead",
            }
        )
    ...
    for source in sources:
        ...
        regex = source["regex"]
        kind_override = source["kind_override"]
        names = client.list_names(remote_dir)
        for name in names:
            parsed = _parse_forecast_meta(name, regex, run_date, kind_override=kind_override)
            if parsed:
                matches.append({**parsed, "remote_dir": remote_dir})
