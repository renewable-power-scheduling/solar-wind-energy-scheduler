...
export function ScheduleTemplates({ context = null }) {
  ...
  useEffect(() => {
    ...
    setAutoConfirmUploadRequested(false);
    handleConfirmUploaded({ skipFreeze: readinessIsDayAhead });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [autoConfirmUploadRequested, isSldcReady, confirmingSldc, readinessIsDayAhead]);
      ...
      <div className="max-w-[1600px] mx-auto p-4 sm:p-6 space-y-6 w-full">
        <div className="rounded-xl border border-border bg-card p-4 sm:p-5">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-2">
            Each site can have its own SLDC template. Pick plant + date, fetch latest frozen S3 schedule, preview conversion, then generate and download.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 sm:p-5 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2">
            ...
            <button
              onClick={onPreview}
              disabled={!canPreview}
              className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md bg-primary text-primary-foreground disabled:opacity-50 ${isSldcReady ? 'opacity-60' : ''}`}
            >
            ...
            <button
              ...
              className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md bg-success text-white disabled:opacity-50 ${isSldcReady ? 'opacity-60' : ''}`}
            >
            ...
            <button
              ...
              disabled={!selectedPlantCode}
              aria-disabled={!isSldcReady}
              className={`w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md border ${
                isSldcReady
                  ...
                  : 'bg-success/10 text-success border-success/50 hover:bg-success/15 opacity-70'
              } disabled:opacity-50`}
            >
