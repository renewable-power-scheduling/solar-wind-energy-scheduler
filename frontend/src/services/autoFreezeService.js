...
export async function autoFreezeFromScheduleKey(
  ...
  if (orderedManualLayers.length) {
    const built = await buildManualFrozenFromLayers({
      plantCode,
  ...
  let editedSourceMap = toDaSourceMap(baseFileName);
  if (orderedManualLayers.length) {
    const built = await buildManualFrozenFromLayers({
      plantCode,
      scheduleDate: operatingDate,
    ...
    if (existingEditedBase?.rows?.length) {
      editedScheduleMap = toScheduleMap(existingEditedBase.rows);
      editedSourceMap = new Map();
      for (let block = 1; block <= TOTAL_BLOCKS; block += 1) {
        const rawSource = existingEditedBase.sourceMap?.get?.(block) || '';
        editedSourceMap.set(block, normalizeEditedSourceLabel(rawSource, baseFileName));
        if (!editedScheduleMap.has(block)) editedScheduleMap.set(block, 0);
      }
    } else {
      editedScheduleMap = toScheduleMap(dayAheadRows);
      editedSourceMap = toDaSourceMap(baseFileName);
      for (let block = 1; block <= TOTAL_BLOCKS; block += 1) {
        if (!editedScheduleMap.has(block)) editedScheduleMap.set(block, 0);
      }
    }
