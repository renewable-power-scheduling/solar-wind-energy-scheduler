...
class ScheduleReadinessService:
    ...
    def check_all_plants(self) -> Dict[str, int]:
        ...
        for plant in plants:
            plant_any = cast(Any, plant)
            plant_id = int(getattr(plant_any, "id", 0) or 0)
            self.check_plant_readiness(plant_id)
            # Get updated status
            readiness = self.get_plant_readiness(plant_id)
            if readiness:
                readiness_any = cast(Any, readiness)
                status = str(getattr(readiness_any, "status", "") or "")
    ...
    def check_plant_readiness(self, plant_id: int) -> Optional[ScheduleReadiness]:
        """
        ...
        """
        plant = self.db.query(Plant).filter(Plant.id == int(plant_id)).first()
        if not plant:
            return None
        
        # Get existing readiness or create new
        readiness = self.db.query(ScheduleReadiness).filter(
            ScheduleReadiness.plant_id == int(plant_id)
        ).first()
        ...
        # Pylance: SQLAlchemy models are typed as Column[...] at the class level.
        # Cast to Any to work with instance attributes without false-positive type errors.
        readiness_any = cast(Any, readiness)
        plant_any = cast(Any, plant)
        
        # Check for triggers
        weather_trigger = self._check_weather_change(int(plant_id))
        curtailment_trigger = self._check_curtailment(int(plant_id))
        deviation_trigger = self._check_deviation(int(plant_id))
        ...
        if new_status == "PENDING" and active_triggers:
            trigger_types = list({str(getattr(t, "trigger_type", "") or "") for t in active_triggers if t})
            trigger_reason = ", ".join([t for t in trigger_types if t])
            
            # Create notifications for triggers
            for trigger in active_triggers:
                self._create_notification(int(plant_id), str(getattr(plant_any, "name", "") or ""), trigger)
        
        # Update readiness record
        old_status = str(getattr(readiness_any, "status", "") or "")
        readiness_any.status = str(new_status)
        readiness_any.last_checked = datetime.now()
        readiness_any.trigger_reason = trigger_reason
        
        # If status changed to READY, set upload deadline
        if new_status == "READY" and old_status != "READY":
            readiness_any.upload_deadline = datetime.now() + timedelta(hours=self.UPLOAD_DEADLINE_HOURS)
            readiness_any.revision_number = int(getattr(readiness_any, "revision_number", 0) or 0) + 1
            
            # Create notification for READY status
            self._create_ready_notification(int(plant_id), str(getattr(plant_any, "name", "") or ""), readiness_any.upload_deadline)
        
        # If status changed to NO_ACTION, clear trigger reason
        if new_status == "NO_ACTION":
            readiness_any.trigger_reason = None
    ...
    def _check_deviation(self, plant_id: int) -> Optional[ScheduleTrigger]:
        ...
        # Parse block data
        try:
            meter_any = cast(Any, meter_data)
            raw_block = getattr(meter_any, "blockData", None)
            block_data = json.loads(raw_block) if isinstance(raw_block, str) else raw_block
    ...
    def _check_weather_change(self, plant_id: int) -> Optional[ScheduleTrigger]:
        ...
        # Parse forecast data
        try:
            weather_any = cast(Any, weather)
            raw_forecast = getattr(weather_any, "forecast", None)
            forecast_data = json.loads(raw_forecast) if isinstance(raw_forecast, str) else raw_forecast
        except (json.JSONDecodeError, TypeError):
            return None
        ...
        plant_any = cast(Any, plant)
        plant_type = str(getattr(plant_any, "type", "") or "")
        if plant_type == "Wind":
            # Check wind speed change
            current_wind = float(getattr(weather_any, "windSpeed", 0) or 0)
            ...
            current_cloud = float(getattr(weather_any, "cloudCover", 0) or 0)
    ...
    def _check_curtailment(self, plant_id: int) -> Optional[ScheduleTrigger]:
        ...
        whatsapp_any = cast(Any, whatsapp_data)
        if bool(getattr(whatsapp_any, "curtailmentStatus", False)):
            trigger = ScheduleTrigger(
                plant_id=plant_id,
                trigger_type="Curtailment",
                severity="CRITICAL",
                description=f"Curtailment active: {str(getattr(whatsapp_any, 'curtailmentReason', '') or '') or 'No reason specified'}",
                actual_value=1
            )
            self.db.add(trigger)
            self.db.commit()
    ...
    def _create_notification(
        ...
        trigger_type = str(getattr(trigger_any, "trigger_type", "") or "")
        description = str(getattr(trigger_any, "description", "") or "")
        actual_value = getattr(trigger_any, "actual_value", 0)
        severity = str(getattr(trigger_any, "severity", "") or "")

        notification = ScheduleNotification(
            plant_id=plant_id,
            plant_name=plant_name,
            notification_type="Trigger Alert",
            title=title_map.get(trigger_type, "Trigger Detected"),
            message=message_map.get(trigger_type, description),
    ...
    def continue_existing_schedule(self, plant_id: int) -> Optional[ScheduleReadiness]:
        ...
        if readiness:
            import typing
            readiness_mut = typing.cast(typing.Any, readiness)
            readiness_mut.status = "NO_ACTION"
            # Keep a concrete string to avoid nullability/type-checker issues.
            readiness_mut.trigger_reason = ""
            readiness_mut.last_checked = datetime.now()
            self.db.commit()
            self.db.refresh(readiness)
    ...
    def mark_schedule_ready(
        ...
        readiness_any.status = "READY"
        readiness_any.upload_deadline = upload_deadline or (datetime.now() + timedelta(hours=self.UPLOAD_DEADLINE_HOURS))
        readiness_any.revision_number = int(getattr(readiness_any, "revision_number", 0) or 0) + 1
        readiness_any.last_checked = datetime.now()
        
        # Create notification
        self._create_ready_notification(plant_id, str(getattr(plant_any, "name", "") or ""), readiness_any.upload_deadline)
    ...
    def mark_notification_read(self, notification_id: int) -> Optional[ScheduleNotification]:
        ...
        if notification:
            notification_any = cast(Any, notification)
            notification_any.read = True
            self.db.commit()
