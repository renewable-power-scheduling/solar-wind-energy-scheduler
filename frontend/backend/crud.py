...
def get_schedule_with_blocks(db: Session, schedule_id: int) -> Optional[Dict[str, Any]]:
    ...
    schedule_any = cast(Any, db_schedule)
    
    schedule_dict = {
        "id": schedule_any.id,
        "plantName": schedule_any.plantName,
        "type": schedule_any.type,
        "scheduleDate": schedule_any.scheduleDate,
        "capacity": schedule_any.capacity,
        "forecasted": schedule_any.forecasted,
        "actual": schedule_any.actual,
        "status": schedule_any.status,
        "deviation": schedule_any.deviation,
        "createdAt": schedule_any.createdAt
    }
    
    # Parse blockData
    raw_block = str(getattr(schedule_any, "blockData", "") or "")
    if raw_block:
        try:
            schedule_dict["blockData"] = json.loads(raw_block)
...
def create_deviation_from_schedule(db: Session, schedule: Schedule) -> List[Deviation]:
    ...
    schedule_any = cast(Any, schedule)
    raw_block = str(getattr(schedule_any, "blockData", "") or "")
    if not raw_block:
        return []
    
    try:
        blockData = json.loads(raw_block)
    except json.JSONDecodeError:
        return []
...
def get_reports(db: Session, skip: int = 0, limit: int = 100, type: Optional[str] = None, state: Optional[str] = None) -> List[Report]:
    ...
    for report in reports:
        ...
        created_at = getattr(report_any, "createdAt", None)

        if status == "Generating" and file_path:
            if os.path.exists(file_path):
                report_any.status = "Ready"
                db.commit()
                db.refresh(report_any)
            elif created_at:
                # If file doesn't exist and report is old (more than 5 minutes), mark as Failed
                if (datetime.now() - created_at).total_seconds() > 300:
                    report_any.status = "Failed"
                    db.commit()
                    db.refresh(report_any)
...
def update_report(db: Session, report_id: int, filePath: Optional[str] = None, status: Optional[str] = None) -> Optional[Report]:
    ...
    report_any = cast(Any, db_report)
    
    if filePath is not None:
        report_any.filePath = filePath
    if status is not None:
        report_any.status = status
...
def get_dashboard_stats(db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        pending_schedules = [s for s in all_schedules if str(getattr(cast(Any, s), "status", "") or "") == "Pending"]
        approved_schedules = [s for s in all_schedules if str(getattr(cast(Any, s), "status", "") or "") == "Approved"]
        revised_schedules = [s for s in all_schedules if str(getattr(cast(Any, s), "status", "") or "") == "Revised"]
        # Active schedules for generation calculation
        active_schedules = [s for s in all_schedules if str(getattr(cast(Any, s), "status", "") or "") == "Active"]
        
        active_plants = [p for p in plants if str(getattr(cast(Any, p), "status", "") or "") == "Active"]
        wind_plants = [p for p in active_plants if str(getattr(cast(Any, p), "type", "") or "") == "Wind"]
        solar_plants = [p for p in active_plants if str(getattr(cast(Any, p), "type", "") or "") == "Solar"]
        
        # Handle null capacity values safely
        total_capacity = sum(float(getattr(cast(Any, p), "capacity", 0) or 0) for p in active_plants)
        wind_capacity = sum(float(getattr(cast(Any, p), "capacity", 0) or 0) for p in wind_plants)
        solar_capacity = sum(float(getattr(cast(Any, p), "capacity", 0) or 0) for p in solar_plants)

        current_generation = 0.0
        for sched in active_schedules:
            actual_val = getattr(cast(Any, sched), "actual", None)
            if actual_val is None:
                continue
            try:
                current_generation += float(actual_val)
            except Exception:
                continue
...
def get_schedule_readiness_summary(db: Session) -> Dict[str, Any]:
    ...
    ready_count = len([r for r in all_readiness if str(getattr(cast(Any, r), "status", "") or "") == "READY"])
    pending_count = len([r for r in all_readiness if str(getattr(cast(Any, r), "status", "") or "") == "PENDING"])
    no_action_count = len([r for r in all_readiness if str(getattr(cast(Any, r), "status", "") or "") == "NO_ACTION"])
...
def acknowledge_schedule_trigger(db: Session, trigger_id: int) -> Optional[ScheduleTrigger]:
    ...
    trigger_any = cast(Any, db_trigger)
    trigger_any.acknowledged = True
    db.commit()
    db.refresh(db_trigger)
    return db_trigger
...
def mark_notification_read(db: Session, notification_id: int) -> Optional[ScheduleNotification]:
    ...
    notification_any = cast(Any, db_notification)
